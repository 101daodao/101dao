<script setup>
const info = [
  { icon: 'School', label: '学校', value: '辽宁建筑职业学院' },
  { icon: 'Collection', label: '专业', value: '软件技术（24级）' },
  { icon: 'Message', label: '邮箱', value: '3550420565@qq.com' },
  { icon: 'ChatDotRound', label: '微信', value: 'daow101' },
]
</script>

<template>
  <section id="about" class="section">
    <h2 class="section-title fade-up">关于我</h2>
    <p class="section-subtitle fade-up delay-1">了解更多关于我的故事</p>

    <div class="about-grid">
      <div class="avatar-area fade-left delay-2">
        <div class="avatar-ring">
          <img src="../image/tx.jpg" alt="头像" class="avatar" loading="lazy" />
        </div>
        <h3 class="name">周</h3>
        <span class="title-tag">在校学生</span>
      </div>
      <div class="info-area fade-right delay-2">
        <h3>一位热爱前端的开发者</h3>
        <p v-typewriter class="bio">
         嗨，我是周，一个写 Vue 的前端开发。
从大一下手写第一个 HTML 页面开始，我就喜欢上了那种"写几行代码就能看到效果"的感觉。现在主要用 Vue 3 全家桶干活，独立做过 AI 智学系统和云音乐 Web 端，从路由设计到接口联调都自己来。
        </p>
        <p v-typewriter class="bio">
          写代码之外，我更关注 AI 怎么改变开发本身。日常用 Vibe Coding 的方式工作——用自然语言描述意图，让 AI 生成代码，我来审查、调试、兜底。参加过 AI 创作大赛，也在尝试 RAG 这类 AI 应用方向。对我来说，AI 不是替代，是杠杆：一个人也能做出以前一个团队才搞定的东西。
工程化不是可选项。Git commit 粒度要细、ESLint 零警告才提交、Vite 构建配置自己掌控。Vibe Coding 产出快，但没有工程化兜底，快就是乱。所以我的节奏是：AI 生成 → 人工审查 → 规范约束 → 可维护交付。
        </p>
        <div class="highlights">
          <div class="hl-item" v-for="(h, i) in ['🏆 项目驱动学习', '⚙️ 工程化开发', '🚀 快速成长']" :key="i">
            {{ h }}
          </div>
        </div>
        <div class="info-list">
          <div v-for="item in info" :key="item.label" class="info-item">
            <el-icon :size="16" color="var(--primary)"><component :is="item.icon" /></el-icon>
            <span class="label">{{ item.label }}</span>
            <span class="value">{{ item.value }}</span>
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

<style scoped>
.about-grid {
  display: grid; grid-template-columns: 260px 1fr; gap: 48px; align-items: start;
}
.avatar-area {
  display: flex; flex-direction: column; align-items: center; gap: 12px;
}
.avatar-ring {
  width: 200px; height: 200px; border-radius: 50%; padding: 4px;
  background: linear-gradient(135deg, var(--primary), var(--primary-light));
  box-shadow: 0 12px 40px rgba(37,99,235,.2);
  animation: ring-pulse 3s ease-in-out infinite;
}
@keyframes ring-pulse {
  0%, 100% { box-shadow: 0 12px 40px rgba(37,99,235,.2); }
  50% { box-shadow: 0 12px 50px rgba(37,99,235,.35); }
}
.avatar {
  width: 100%; height: 100%; border-radius: 50%;
  object-fit: cover; object-position: center top;
  display: block;
}
.name { font-size: 22px; font-weight: 700; margin: 0; }
.title-tag {
  padding: 4px 14px; border-radius: 20px; font-size: 13px; font-weight: 500;
  background: rgba(37,99,235,.08); color: var(--primary);
}
.info-area h3 { font-size: 22px; font-weight: 700; margin-bottom: 16px; }
.bio {
  color: var(--text-secondary); line-height: 1.8; margin-bottom: 12px; font-size: 15px;
}
.highlights { display: flex; gap: 12px; margin: 20px 0; flex-wrap: wrap; }
.hl-item {
  padding: 8px 16px; border-radius: 8px;
  background: var(--bg-soft); font-size: 13px; font-weight: 500;
  transition: transform .2s, box-shadow .2s;
}
.hl-item:hover {
  transform: translateY(-2px);
  box-shadow: 0 4px 12px rgba(37,99,235,.1);
}
.info-list { display: flex; flex-direction: column; gap: 10px; }
.info-item {
  display: flex; align-items: center; gap: 10px; font-size: 14px;
}
.info-item .label { color: var(--text-secondary); min-width: 40px; }
.info-item .value { color: var(--text); font-weight: 500; }

@media (max-width: 768px) {
  .about-grid { grid-template-columns: 1fr; text-align: center; }
  .avatar-ring { width: 150px; height: 150px; }
  .hl-item { justify-content: center; }
  .info-item { justify-content: center; }
}
</style>
